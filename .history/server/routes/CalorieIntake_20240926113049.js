...
router.post('/getcalorieintakebylimit', authTokenHandler, async (req, res) => {
    ...
    // If the limit is 'all', return all calorie intake entries
    else if (limit === 'all') {
        return res.json(createResponse(true, 'Calorie intake', user.calorieIntake));
    } 
    
    // If a valid numeric limit is provided, filter calorie intake data
    else {
        let date = new Date();
        let currentDate = new Date();
        let comparisonDate = new Date(currentDate);
        comparisonDate.setDate(currentDate.getDate() - parseInt(limit)); 
