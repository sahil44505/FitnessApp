...
router.post('/getcalorieintakebylimit', authTokenHandler, async (req, res) => {
    ...
    if (!limit) {
        ...
        let date = new Date();
        date.setHours(0, 0, 0, 0); // Set time to 00:00:00 for accurate comparison
        let currentDate = new Date(date.setDate(date.getDate() - parseInt(limit))).getTime();

        // Filter the calorie intake array based on the calculated comparison date
        const filteredCalorieIntake = user.calorieIntake.filter((data) => {
            return new Date(data.date).getTime() >= currentDate;
        });

        return res.json(createResponse(true, `Steps entries for the last ${limit} days`,filteredCalorieIntake));
    }
});
