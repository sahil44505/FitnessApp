...
let weightInKg = (user.tracker && user.tracker.length > 0 && user.tracker[0].heightInCm && user.tracker[0].heightInCm.length > 0) 
    ...
    if (user.tracker && user.tracker.length > 0 && user.tracker[0].dob) {
        birthDate = new Date(user.tracker[0].dob);
        ...
        // Set age to undefined if there's no dob available
        age = undefined; // Change this to NaN or any value according to your requirements
    }
    let BMR = 0;
