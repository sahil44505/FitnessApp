...
router.delete('/deletecalorieintake', authTokenHandler, async (req, res) => {
    ...
    if (!date) {
        let date = new Date();   // sept 1 2021 12:00:00
        user.calorieIntake.item = filterEntriesByDate(user.calorieIntake.item, date);

        return res.json(createResponse(true, 'Calorie intake for today', user.calorieIntake));
    }
    user.calorieIntake.item = filterEntriesByDate(user.calorieIntake.item, new Date(date));
    console.log(user.calorieIntake.item)

    console.log('Item:', item, 'Date:', date);
    console.log('Current calorie intake:', user.calorieIntake); 

    // Filter out the item to be removed
    const updatedCalorieIntake = user.calorieIntake.filter(entry => 
        !(entry.item === item && entry.date === date)
    );

    // Update the user's calorie intake with the filtered array
    const result = await collection.updateOne(
        { _id: userId },
        { $set: { calorieIntake: updatedCalorieIntake } }
    );
