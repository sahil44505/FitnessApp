...
router.delete('/deletecalorieintake', authTokenHandler, async (req, res) => {
    ...
    // Convert the provided date to a Date object and normalize it to only the date part
    const targetDate = new Date(date);
    const targetDateStr = targetDate.toISOString().split('T')[0]; // Get YYYY-MM-DD part of the date

    // Log user.calorieIntake for debugging
    console.log('Before filtering:', user.calorieIntake);

    // Filter out the matching calorie intake entry by item and date (only the date part)
    user.calorieIntake = user.calorieIntake.filter((entry) => {
        const entryDateStr = new Date(entry.date).toISOString().split('T')[0]; // Normalize the date in calorieIntake
        
        // Log each entry's date and comparison with target date for debugging
        console.log(`Comparing entry: ${entry.item} on date: ${entryDateStr} with target date: ${targetDateStr}`);
        
        return !(entry.item === item && entryDateStr === targetDateStr);
    });

    // Log user.calorieIntake after filtering
    console.log('After filtering:', user.calorieIntake);
    ...
    if (result.modifiedCount === 0) {
        console.log('No matching calorie intake found to delete.');
