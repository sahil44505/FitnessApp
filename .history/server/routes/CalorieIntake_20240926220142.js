    ...
    request.get({
        ...
        if (error) return console.error('Request failed:', error);
            ...
            const result = await collection.updateOne(
              ...
              { _id: userId },
                { $push: { 'tracker.0.caloreiIntake': newCalorieEntry }}
...
router.post('/getcalorieintakebydate', authTokenHandler, async (req, res) => {
    ...
    if (!date) {
        ...
        user.calorieIntake = filterEntriesByDate(user.tracker[0].calorieIntake, date);

        return res.json(createResponse(true, 'Calorie intake for today', user.calorieIntake));
    }
    user.calorieIntake = filterEntriesByDate(user.tracker[0].calorieIntake, new Date(date));
...
router.post('/getcalorieintakebylimit', authTokenHandler, async (req, res) => {
    ...
    if (!limit) {
        ...
        user.calorieIntake = user.calorieIntake.filter((data) => {
            return new Date(data.date).getTime() >= currentDate;
        })

        return res.json(createResponse(true, `Steps entries for the last ${limit} days`, user.calorieIntake));
    }
});
...
router.delete('/deletecalorieintake', authTokenHandler, async (req, res) => {
    ...
    user.calorieIntake = user.calorieIntake.filter((item) => {
        return item.item !== itemtoremove && item.date !== date; // Example with 'juice'
    });
