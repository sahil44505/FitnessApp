...
const Reports = () => {
  ...
  const getDataForS1 = async () => {
    if (pathname === '/report/Calorie%20Intake') {
      ...
      const token = localStorage.getItem('token');
      await fetch('http://localhost:8080/step/getstepsbylimit', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`,
  ...
  const setDataForGraphs = (data, limit, setStateFunc,feild) => {
    const limitedData = limit === 'all' ? data : data.slice(0, limit);
    const temp = limitedData.map((item) => ({
      date: item.date,
      value: item[field],  // Use dynamic field (calorieIntake or steps)
      unit: field === 'calorieIntake' ? 'kcal' : 'steps',
    }));
