...
router.post('/getweightbydate', authTokenHandler, async (req, res) => {
    ...
    const user = await collection.findOne({ _id: userId });
    console.log(user.tracker[0].weightInKg)

    if (!date) {
        let date = new Date();
        user.tracker.weightInKg = filterEntriesByDate(user.tracker[0].weightInKg, date);

        return res.json(createResponse(true, 'Weight entries for today', user.tracker[0].weightInKg));
    }

    user.tracker.weightInKg = filterEntriesByDate(user.weightInKg, new Date(date));
    res.json(createResponse(true, 'Weight entries for the date', user.tracker[0].weightInKg));
});
