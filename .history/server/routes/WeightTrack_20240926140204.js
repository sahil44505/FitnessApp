...
router.post('/getweightbydate', authTokenHandler, async (req, res) => {
    ...
    if (!date) {
        let date = new Date();
        user.tracker.weightInKg = filterEntriesByDate(user.tracker.weightI, date);

        return res.json(createResponse(true, 'Weight entries for today', user.tracker.weight));
    }

    user.weight = filterEntriesByDate(user.tracker.weight, new Date(date));
    res.json(createResponse(true, 'Weight entries for the date', user.tracker.weight));
});
