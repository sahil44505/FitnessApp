...
router.post('/getcalorieintakebydate', authTokenHandler, async (req, res) => {
    ...
    const filteredEntries = user.tracker[0].calorieIntake.filter(entry => {
        const entryDateStr = new Date(entry.date).toISOString().split('T')[0]; // Get date in 'YYYY-MM-DD' format
        return entryDateStr === inputDateStr; // Compare only the date parts
    });

    if (filteredEntries.length === 0) {
        return res.json(createResponse(false, 'No calorie intake data for the provided date', []));
    }
...
router.delete('/deletecalorieintake', authTokenHandler, async (req, res) => {
    ...
    user.tracker.calorieIntake = user.tracker[0].calorieIntake.filter((entry) => {
        ...
        return !(entry.item === item && new Date(entry.date).toISOString() === inputDate);
    });
