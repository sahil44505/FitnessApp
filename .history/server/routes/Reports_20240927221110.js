...
router.get('/getreport', authTokenHandler, async (req, res) => {
    ...
    let today = new Date();
        ...
        console.log('Today\'s Date in ISO format (YYYY-MM-DD):', todayISO);
   console.log(user.tracker[0].calorieIntake[0])
    user.tracker[0].calorieIntake.forEach((entry) => {
        let entryDateISO = new Date(entry.date).toISOString().split('T')[0];

            console.log('Entry Date in ISO format (YYYY-MM-DD):', entryDateISO);
            console.log('Entry Calorie Intake:', entry.calorieIntake);

            // Compare the ISO date strings
            if (entryDateISO === todayISO) {
                calorieIntakee += entry.calorieIntake;
            }
    });
