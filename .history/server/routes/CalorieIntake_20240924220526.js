...
router.delete('/deletecalorieintake', authTokenHandler, async (req, res) => {
    ...
    user.calorieIntake = user.calorieIntake.filter((entry) => {
        ...
        // Ensure item comparison is case insensitive
        const itemMatch = entry.item.toLowerCase() === item.toLowerCase();
        const dateMatch = entryDateStr === targetDateStr;

        // Log match results for debugging
        console.log(`Item match: ${itemMatch}, Date match: ${dateMatch}`);
