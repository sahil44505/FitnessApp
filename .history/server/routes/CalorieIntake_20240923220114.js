...
router.get('/getgoalcalorieintake', authTokenHandler, async (req, res) => {
    ...
    let heightInCm = parseFloat(user.tracker.heightInCm[user.tracker.heightInCm.length - 1].heightInCm);
    let weightInKg = parseFloat(user.tracker.weightInKg[user.tracker.weightInKg.length - 1].weight);
