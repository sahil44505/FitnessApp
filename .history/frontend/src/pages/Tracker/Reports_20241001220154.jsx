        ...
        .then((data) => {
          if (data.ok) {
            if (data.ok && Array.isArray(data.data)) {
              // Sort only if data is valid
              let sortedData = data.data.sort((a, b) => new Date(b.date) - new Date(a.date));
            // Set data for each graph limit
            setDataForGraphs(sortedData, 10, setData10Days);
            setDataForGraphs(sortedData, 30, setData30Days);
            setDataForGraphs(sortedData, 365, setData365Days);
            setDataForGraphs(sortedData, 'all', setDataAllTime);
            }
          } else {
