...
router.delete('/deletecalorieintake', authTokenHandler, async (req, res) => {
    ...
    // Filter the calorieIntake array
    const updatedCalorieIntake = user.calorieIntake.filter(calorie => {
        return !(calorie.item === item && new Date(calorie.date).getTime() === formattedDate.getTime());
    });

    // Update the user's calorieIntake in the database
