...
router.delete('/deletecalorieintake', authTokenHandler, async (req, res) => {
    ...
    // Parse the date sent from Postman into a Date object
    const targetDate = new Date(date);
    if (isNaN(targetDate)) {
        return res.status(400).json(createResponse(false, 'Invalid date format.'));
    }

    // Normalize the target date to YYYY-MM-DD
    const targetDateStr = targetDate.toISOString().split('T')[0]; // Get the YYYY-MM-DD part of the date

    // Log the calorieIntake before filtering for debugging
    console.log('Before filtering:', user.calorieIntake);

    // Filter the calorieIntake entries by comparing only the date part
    user.calorieIntake = user.calorieIntake.filter((entry) => {
        const entryDateStr = new Date(entry.date).toISOString().split('T')[0]; // Normalize the entry date
    ...
    // Log the calorieIntake after filtering
