...
router.delete('/deletecalorieintake', authTokenHandler, async (req, res) => {
    ...
    consolr.log(parsedDate)
    // Convert to ISO string to match the format in MongoDB
    const isoDateString = parsedDate.toISOString();
    console.log(isoDateString)

    // Update the calorieIntake array
    const result = await collection.updateOne(
        { _id: userId },
        { $pull: { calorieIntake: { item: item, date: isoDateString } } }
    );

    if (result.modifiedCount === 0) {
        return res.status(404).json(createResponse(false, 'No matching calorie intake found'));
    }
