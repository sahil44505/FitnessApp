...
const Step = ({ setShowStepPopup }) => {
  ...
  const handleSave = async() => {
    ...
    await fetch('http://localhost:8080/calorie/addSteps', {
      method: 'POST',
      headers: {
      ...
      body: JSON.stringify({
        ...
        quantitytype:Steps.quantitytype
      })
  })
  .then(res => res.json())
  .then(data =>{
  ...
  const getSteps = async()=>{
    setItems([])
    const token = localStorage.getItem('token')
    await fetch('http://localhost:8080/calorie/getStepsbydate', {
      method: 'POST',
      headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
  ...
  const handleDelete = async(item) => {
    ...
    const token = localStorage.getItem('token')
//     await fetch('http://localhost:8080/calorie/deleteSteps', {
//       method: 'DELETE',
//       headers: {
//           'Content-Type': 'application/json',
//           'Authorization': `Bearer ${token}`
    ...
    <div className='popupout'>
      <div className='popupbox'>
        ...
        <TextField
          ...
          onChange={(e) => setSteps({...Steps,quantity:e.target.value})}
        />
