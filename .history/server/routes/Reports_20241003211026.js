...
router.get('/getreport', authTokenHandler, async (req, res) => {
    ...
    // get today's weight
    let weight = user.tracker[0].weightInKg && user.tracker[0].weightInKg.length > 0 
        ? user.tracker[0].weightInKg[user.tracker[0].weightInKg.length - 1].weight 
        : undefined; 

    // Get today's height
    let height = user.tracker[0].heightInCm && user.tracker[0].heightInCm.length > 0 
    ...
    workout = undefined; // Change to another value as per your requirements
}
