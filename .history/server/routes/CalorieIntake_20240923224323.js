...
router.delete('/deletecalorieintake', authTokenHandler, async (req, res) => {
    ...
    const user = await collection.findOne({ _id: userId });
    console.log('Current calorie intake:', user.calorieIntake); // Log the current calorie intake

    // Log the incoming request data
    console.log('Item:', item, 'Date:', isoDate);

    // Update the user's calorie intake
    const result = await collection.updateOne(
        { _id: userId },
        { $pull: { calorieIntake: { item: item, date: isoDate } } }
    );

    console.log('Deletion result:', result); // Log the result of the deletion attempt
